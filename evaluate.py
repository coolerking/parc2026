...
_LIBERO_SUITES_PATH = "/workspace/compe"
...
def start_policy_server(submission_dir: str, port: int) -> subprocess.Popen:
...
def wait_for_server(port: int, timeout: int | None = None) -> bool:
    if timeout is None:
...
def run_pipeline(
    ...
    env = os.environ.copy()
    env["PYTHONPATH"] = (
        f"{_LIBERO_PATH}:{_PIPELINE_PATH}:{_LIBERO_SUITES_PATH}:"
    ...
    try:
        ...
        if _sb is not None:
            server_proc = _sb.start_server_process(
                server_python, submission_root, port, drop_to, sys.stderr)
        else:
            server_proc = start_policy_server(submission_root, port)


        if not wait_for_server(port):
            if server_proc.poll() is not None:
                log("エラー: ポリシーサーバーが起動に失敗しました")
                json.dump(
                    error_result("ポリシーサーバーが起動に失敗しました"), sys.stdout
                )
            else:
                log("エラー: サーバーが制限時間内に起動しませんでした")
                json.dump(
                    error_result("サーバーが制限時間内に起動しませんでした"), sys.stdout
                )
            sys.exit(1)
